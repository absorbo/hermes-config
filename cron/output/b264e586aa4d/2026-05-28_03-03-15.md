...
Analysis complete. Here's the final categorization:

- **8 of 9 "broken" links are false positives** — all 4 template files (`Control Template.md`, `Evidence Template.md`, `Gap Template.md`, `Action Template.md`) exist in `90 - Templates/CyFUN/` (verified with `ls`). They were flagged only because the audit script excludes `90 - Templates/` from indexing per the rules. These links resolve correctly in Obsidian.

- **1 genuinely broken link** — `Clawbot Vault MOC.md` line 86: `[[70 - Writing/03_Output/vienna-protocol/chapters/|Chapters]]` — the trailing `/` makes this link to a directory, which Obsidian doesn't resolve. The chapters directory contains 17+ `.md` files but no single obvious target for repair. **Needs human attention.**

No repairs performed — the single genuine break has no unambiguous alternate target.
...
|---|---|
| MOCs checked | 14 |
| Total wikilinks verified | 258 |
| Broken links (genuine) | 1 |
| False positives (template exclusion) | 8 |
| Repaired | 0 |
| Remaining issues | 1 |
...
| MOC | Links | Broken |
|---|---|---|
| Clawbot Vault MOC | 35 | **1** |
| Knowledge MOC | 39 | 0 |
| CyFUN MOC (Knowledge) | 12 | 0* |
| Integrated Controls Management Overview | 5 | 0 |
| Customers MOC | 19 | 0 |
| CyFUN MOC (Verlinfo) | 12 | 0* |
| Entities MOC | 15 | 0 |
| Personal MOC | 3 | 0 |
| Intermediaries MOC | 31 | 0 |
| CRM MOC | 10 | 0 |
| Reference MOC | 24 | 0 |
| Content Pipeline MOC | 13 | 0 |
| Writing Studio MOC | 35 | 0 |
| Tags MOC | 5 | 0 |

*\*CyFUN MOCs have template links flagged as broken by exclusion, but files verified to exist.*

### Remaining issue
- **`Clawbot Vault MOC.md` line 86**: `[[70 - Writing/03_Output/vienna-protocol/chapters/|Chapters]]` — links to a directory (trailing `/`), which Obsidian doesn't resolve. 17+ chapter files exist in the directory; no single obvious replacement target.
