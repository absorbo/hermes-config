...
**GraphWeaver MOC Integrity Audit — 2026-05-26**

| Metric | Count |
|--------|-------|
| MOC/Overview files checked | 35 |
| Wikilinks verified | 79 |
| Resolved | 68 |
| Broken | 11 |
| Repaired | **0** |
| Files modified | **0** |
| Audit note created | **1** (`Audit/MOC Integrity Audit 2026-05-26.md`) |

**Skipped / Unrepairable:**
- 11 broken links are dead targets — the referenced notes do not exist anywhere in the vault under any path variant. Per constraints, no speculative edits were made.
- 2 audit output files from prior runs are orphaned (expected).

**Blockers / Issues:**
- **11 persistent dead links** across 5 entity/customer overviews. These are placeholder task links (`Task-Ensure-Payment`, `Project-Go-To-Market`, `KE-002-ISO27001`, etc.) that were never created.
- **2 cross-reference gaps:** `Loose Maarten CommV/00-Overview.md` is not linked from `Entities MOC.md`; `ModelOverviewv2.md` is not linked from `Knowledge MOC.md`.

No files were changed. No templates or archive files were touched.
