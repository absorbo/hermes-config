...
## MOC Integrity Audit — Clawbot Vault

| Metric | Value |
|--------|-------|
| MOC/Overview files scanned | **32** |
| Total wikilinks verified | **381** |
| Broken links found | **11** |
| Repaired | **0** |
| Remaining issues | **11** (all dead targets) |

### Broken links (all truly dead — no files exist under any path variant)

**`10 - Customers/Delvaux/00-Overview.md`** (1)
- `[[KE-002-ISO27001]]` — no file with this basename anywhere in vault

**`11 - Entities/Eldenwyck-Co/00-Overview.md`** (1)
- `[[11-Entities/Eldenwyck-Co/Task-Ensure-Payment]]` — hyphen-path; corrected path also missing

**`11 - Entities/Minotaur-Solutions/00-Overview.md`** (2)
- `[[11-Entities/Minotaur-Solutions/Task-Accounting-Scan]]`
- `[[11-Entities/Minotaur-Solutions/Task-KMOP-Registration]]`

**`11 - Entities/Oxford-Holdings/00-Overview.md`** (1)
- `[[11-Entities/Oxford-Holdings/Task-Accounting-Cleanup]]`

**`11 - Entities/TruSecure/00-Overview.md`** (3 unique, 6 total)
- `[[11-Entities/TruSecure/Project-Go-To-Market]]` (×3)
- `[[11-Entities/TruSecure/Task-Follow-Up]]`
- `[[11-Entities/TruSecure/Customer-Personas]]` (×2)

### Analysis

- **10/11** are hyphen-path variants (`11-Entities` instead of `11 - Entities`), but corrected paths are also dead — these notes were never created.
- **1/11** (`KE-002-ISO27001`) is a bare basename link to a note that doesn't exist.
- **0 repairs** applied per audit constraints (no speculative note creation).
- **18 initial false positives** from `\\|` table-pipe escapes in `Customers MOC.md` and `Writing Studio MOC.md` were correctly filtered — those links all resolve.
