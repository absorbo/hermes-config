...
# MOC Integrity Audit — 2026-06-03

**Status:** Complete. No repairs applied. 0 files modified.
...
|---|---|
| MOCs checked | 32 |
| Total links checked | 393 |
| Resolved | 381 |
| Broken | 12 |
| Repaired | 0 |
| Orphaned MOCs (0 inbound) | 0 |
| Cross-reference gaps | 2 (content-authoring, not auto-repaired) |

## Broken-link composition (all dead targets, no repairable path variant)

| Count | Category | Reason no repair |
|---|---|---|
| 10 | `11-Entities/...` hyphen-path links (4 unique targets × multiple refs in `11 - Entities/TruSecure/00-Overview.md` + 1 each in Eldenwyck-Co, Minotaur-Solutions, Oxford-Holdings) | Path fix `11-Entities` → `11 - Entities` alone won't help: the underlying `.md` files (Task-Ensure-Payment, Task-Accounting-Scan, Task-KMOP-Registration, Task-Accounting-Cleanup, Project-Go-To-Market, Task-Follow-Up, Customer-Personas) **do not exist** in any entity folder. Only `00-Overview.md`, `Meetings/`, `Payment Calendar.md` are present. The Smart Connections `.ajson` index files referencing these names are plugin metadata, not valid wikilink targets. |
| 1 | `[[KE-002-ISO27001]]` in `10 - Customers/Delvaux/00-Overview.md:38` | Knowledge-engineering note never created. No candidate file at any path. |
| 1 | `[[70 - Writing/03_Output/vienna-protocol/chapters/Chapters]]` in `Obsidian Vault MOC.md:86` | Parent file `Chapters.md` never created. Folder contains 20 `chapter-NN-*.md` files with a different naming pattern (singular `chapter-XX-slug.md`, not `Chapters.md`). |

## Cross-reference gaps (reported, not auto-repaired)

- `04 - Knowledge/SCF/Integrated Controls Management Overview` — not referenced from root `Obsidian Vault MOC.md` (only inbound is the prior audit report). Adding a link is content authoring, not a structural repair.
- `70 - Writing/Tags MOC` — not referenced from root `Obsidian Vault MOC.md` (5 inbound from other places). Same reason.

## Verification

- Re-ran `moc_link_audit.py` after analysis: identical results, no regressions.
- File-existence checks confirmed all 9 candidate target paths are absent.
- No files modified, no template/archive files touched, no speculative notes created.

Identical to the 2026-06-02 reference run (393 links / 12 broken / 0 repaired) — the broken set is stable across runs and requires human authoring decisions to resolve (create the missing task/project/personas notes, or remove the dead references).
