...
## MOC Link Integrity Audit — Clawbot Vault
...
| Metric | Count |
|--------|-------|
| MOC files checked | 32 (14 `*MOC.md` + 18 `*Overview.md`) |
| Template/Archive excluded | 1 (`90 - Templates/Project-Overview.md`) |
| Total wikilinks extracted | 381 |
| Code-span false positives filtered | 0 |
| Valid links (resolved) | 373 |
| **Broken links (truly dead)** | **8** |
| Repaired | 0 |
| Remaining | 8 |

### Categories of 373 Valid Links

- **Standard resolvable (exact/.md append):** 340
- **`\\|` table-escape links (Customers MOC):** 12 — all resolve with `.md` appended
- **Continuity file links (Writing Studio MOC):** 3 — all resolve with `.md` appended
- **Template references (CyFUN MOCs):** 8 — files exist at `90 - Templates/CyFUN/*.md` (valid, indexed outside scope)
- **Folder link (Clawbot Vault MOC):** 1 — `70 - Writing/03_Output/vienna-protocol/chapters/` directory exists
- **Cross-MOC wikilinks with display text:** remainder

### 8 Dead Links — No Repairs Possible

All 8 are genuinely dead (no file exists at any path variant). None qualify for repair per audit rules:

| # | Source File | Dead Target |
|---|------------|-------------|
| 1 | `10 - Customers/Delvaux/00-Overview.md:38` | `KE-002-ISO27001` |
| 2 | `11 - Entities/Minotaur-Solutions/00-Overview.md:25` | `11-Entities/Minotaur-Solutions/Task-Accounting-Scan` |
| 3 | `11 - Entities/Minotaur-Solutions/00-Overview.md:26` | `11-Entities/Minotaur-Solutions/Task-KMOP-Registration` |
| 4 | `11 - Entities/TruSecure/00-Overview.md:22,30,46` | `11-Entities/TruSecure/Project-Go-To-Market` (×3) |
| 5 | `11 - Entities/TruSecure/00-Overview.md:29` | `11-Entities/TruSecure/Task-Follow-Up` |
| 6 | `11 - Entities/TruSecure/00-Overview.md:31,47` | `11-Entities/TruSecure/Customer-Personas` (×2) |
| 7 | `11 - Entities/Eldenwyck-Co/00-Overview.md:24` | `11-Entities/Eldenwyck-Co/Task-Ensure-Payment` |
| 8 | `11 - Entities/Oxford-Holdings/00-Overview.md:24` | `11-Entities/Oxford-Holdings/Task-Accounting-Cleanup` |

**Root cause:** Links 2–8 use `11-Entities/` (hyphens) while vault uses `11 - Entities/` (spaces). However, no corresponding files exist at EITHER path variant — these task/project notes were never created. Link 1 is a bare `KE-002-ISO27001` reference with no matching note anywhere.

### Comparison to Prior Audits

| Date | MOCs | Links | Broken | Repaired |
|------|------|-------|--------|----------|
| 2026-05-25 | 31 | 65 | 11 | 0 |
| 2026-05-29 | 32 | 381 | 11 | 0 |
| **2026-05-30** | **32** | **381** | **8** | **0** |

Improvement of 3 from 2026-05-29: the 3 false positives from the `\\|` table-escape handling bug in the prior audit script are now correctly identified as valid. The remaining 8 are the same dead entity task/project links + KE-002 that have never had corresponding files.
