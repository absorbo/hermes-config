...
## MOC Integrity Audit — Clawbot Vault (2026-06-07)

**Status:** No changes. No regressions. No repairs applied.

### Counts
- **MOCs checked:** 32 (unchanged vs 2026-06-03 baseline)
- **Total wikilinks checked:** 390 (down 3 from 393 — link count drift, no action needed)
- **Resolved:** 378
- **Broken:** 12 (identical set to 2026-06-03)
- **Repaired:** 0 (all 12 are dead targets, no repairable path variant)
- **Orphans (zero inbound from non-MOCs):** 1 — `Obsidian Vault MOC.md` (expected: root MOC, references are MOC→MOC, excluded by script design)
- **Cross-ref gaps (MOCs not linked from root MOC):** 19 (up from 2 — these are content-authoring gaps, not auto-repaired per reference)

### Broken Link Composition (unchanged)
- 1 × `KE-002-ISO27001` (Delvaux) — never created
- 10 × entity hyphen-path links (`11-Entities/...` instead of `11 - Entities/...`) — target files (Task-*, Project-*, Customer-Personas) confirmed absent in entity folders (only `00-Overview.md` and `Payment Calendar.md` exist)
- 1 × `vienna-protocol/chapters/Chapters` — `chapters/` dir exists with 19 `chapter-NN-*.md` files but no `Chapters.md`

### Blockers
- None. Script ran clean. No file modifications made. Templates (`90 - Templates/`) and archive (`99 - Archive/`) untouched per constraint.

### Skipped
- 0 — all MOCs were processed. The 12 dead links are reported only; per task constraint, "never create missing target notes" and "repair only where target clearly exists under a slightly different name" — none qualify.
