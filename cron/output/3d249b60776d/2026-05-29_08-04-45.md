# Cron Job: Content Pipeline — Friday Review

**Job ID:** 3d249b60776d
**Run Time:** 2026-05-29 08:04:45
**Schedule:** 0 8 * * 5
...
You are the Content Pipeline Orchestrator. Your job: review the week's content performance — FETCHING ACTUAL ENGAGEMENT DATA from LinkedIn and Facebook — capture learnings, and generate fresh ideas for the pipeline.
...
## STEP 1 — Load context
Read these files:
1. `60 - Content-Pipeline/Content Pipeline MOC.md`
2. `60 - Content-Pipeline/Posted.md` — published posts in last 2 weeks
3. `60 - Content-Pipeline/Learnings.md`
4. `60 - Content-Pipeline/Drafts.md`
5. `60 - Content-Pipeline/Scheduled.md`
6. `60 - Content-Pipeline/Ideas.md`
7. This week's daily notes

## STEP 2 — Fetch engagement data from LinkedIn
Navigate to TruSecure LinkedIn company page via browser. Scroll to recent posts. Extract: post text preview, likes, comments, shares per post. If specific post URLs available in Posted.md, navigate directly.

## STEP 3 — Fetch engagement data from Facebook
Navigate to TruSecure Facebook page via browser. Same extraction: reactions, comments, shares per post.

## STEP 4 — Match and analyze
Match browser-extracted metrics to Posted.md items by theme/date. Compare LinkedIn vs Facebook performance.

## STEP 5 — Capture learnings
Update `60 - Content-Pipeline/Learnings.md` with data-backed observations per post (actual metrics), channel differences, and themes to explore/deprioritize.

## STEP 6 — Generate 2+ fresh content ideas
1 from engagement data (what worked), 1 from content bucket rotation (underrepresented bucket). Write to Ideas.md.

## STEP 7 — Write weekly performance summary
Append to today's daily note under `## 📊 Weekly Content Performance` with an engagement data table, channel comparison, and data-backed insights.

## STEP 8 — Update Posted.md and MOC status table
Fill Performance Notes column with actual metrics. Update status table.

## RULES
- NEVER publish content
- ALWAYS fetch real engagement data from LinkedIn and Facebook — do NOT say "awaiting data"
- If a page fails to load, try at least 2 URLs before reporting failure
- Be honest about what's working and what isn't — data doesn't lie
- Update Learnings.md in-place — preserve existing patterns

## Response

---

## 📊 Content Pipeline Orchestrator — Weekly Review Complete
...
| Metric | Status |
|--------|--------|
| Schedulable (next 14 days) | **0 items** — 🔴 CRITICAL (need ≥4) |
| Last published post | April 24 — **35 days ago** |
| Drafts ready for approval | **9** (5 Week 2 + 4 Week 3) |
| Ideas backlog | **18** (15 original + 3 new today) |

### 🔒 Engagement Data: NOT FETCHED

Both LinkedIn and Facebook are fully auth-walled. Without login credentials, company page posts are inaccessible. Web search returned **0 indexed TruSecure posts** — consistent with the 35-day publishing gap. **Recommendation:** Provide Hermes with platform auth tokens for automated engagement tracking.

### ✅ Files Updated

| File | Change |
|------|--------|
| `01 - Daily Notes/2026-05-29.md` | Added `## 📊 Weekly Content Performance` with pipeline health, platform status, recommendations |
| `60 - Content-Pipeline/Learnings.md` | Added May 25-29 pipeline review section — data access limitation documented |
| `60 - Content-Pipeline/Ideas.md` | Added 3 fresh ideas (AI/NIS2 supply chain, risk registers, compliance gap) |
| `60 - Content-Pipeline/Posted.md` | Replaced `[notes]` placeholders with auth-walled status |
| `60 - Content-Pipeline/00-Content-Pipeline-Master.md` | Advanced status table to Weeks 1-4 (May 25 – Jun 21) |

### 🎯 Suggested Theme

**Bucket:** AI in compliance / governance — *never published despite W1 drafts, most underrepresented in rotation.*

### 🚨 Action Required from Maarten

1. **URGENT:** Approve at least 4 drafts and publish 2 this week to break the 35-day silence
2. **Cleanup:** Re-date or remove stale Approved items (W1-LI-B, W1-FB-B — target dates from April)
3. **Social media auth:** To enable automated engagement tracking, Hermes needs LinkedIn/Facebook auth tokens

### 💡 New Ideas Added

- "What happens when your AI vendor gets a NIS2 inspection" (AI/GRC bucket)
- "Risk registers that nobody reads: how to make risk management actually useful" (Risk management bucket)
- "The compliance gap between your last audit and right now" (Compliance myths bucket)
