# Cron Job: Invoice & Payment Tracker (FAILED)
...
```
RuntimeError: # Weekly Invoice & Payment Tracker — Status (2026-06-08)

## What Was Accomplished

**Data collection (complete):**
- 8 entities inventoried (Active + Dormant): TruSecure, TruSecure-UK, Minotaur Solutions, Oxford Holdings, Eldenwyck & Co, Privacy Services LLC, REAT Europe, Loose Maarten CommV
- M365 + Gmail auth restored (msal installed, symlink created to `/Users/absorbo/Documents/.hermes/.local`)
- Fetched 100 M365 inbox + 100 M365 sent + 200 Gmail inbox + 100 Gmail sent
- Filtered to **19 M365 + 7 Gmail** invoice-related hits, fetched 7 priority message bodies
- Cross-referenced sent items (response status) and daily notes (payment confirmations)

**Files updated:**
- ✅ **`Loose Maarten CommV/Payment Calendar.md`** — major update (frontmatter, status overview, flagged table, 8 new incoming rows, Enwyse payments closed)

## Key Findings

### 🆕 NEW invoices this week
- **WEX BE41140074 — €109.08** (due 26/05/2026, **13 days overdue**) — new WEX invoice, distinct from older BE41127482 €111.11
- **AZ Groeninge 1260080735 — €7.50** (due 05-07-2026) — patient invoice, not yet due
- **Vanheede VF26-066398** — new waste invoice, amount in portal only
- **Deelkenspoorten INV24/1862** — old 2024 tow-truck invoice, Maarten replied 2026-06-02
- **Bol.com payment reminder** — amount not in body
- **Billit/Fidufix** — 2 Peppol deliveries (accountant-related, amount pending)
- **Billit/Vanheede** — Peppol duplicate of VF26-066398

### ✅ Recently paid
- **Enwyse F-2026020482 (€581.22)** + **F-2026027153 (€549.14)** — both paid 2026-06-06 per daily note + Maarten replied to reminder

### 🔴 Unresolved >7 days (flagged)
1. **Leaseline 12267** €2,043.68 — 39 days overdue, Maarten replied 2026-05-18, payment pending
2. **WEX BE41140074** €109.08 — 13 days overdue, 🆕 NEW
3. **WEX BE41127482** €111.11 — 37 days overdue (older balance)

## Pending (interrupted by tool-iteration cap)

- Loose Maarten CommV: Running Totals + Notes section update
- TruSecure Payment Calendar: add NEW Cloudflare domain auto-renewal + Diversus/Billtobox status update
- 5 other entities: re-verify "no activity" status (no change expected, but the "Last updated" stamp should be refreshed)
- Temp file cleanup in `/tmp`
- Commit to `clawbot-vault` (PRIMARY repo) — per SOUL.md Commit & Backup Gate, this must happen via the standard vault commit flow (vault first, then hermes-config mirror)

**Total estimated open across all entities: ~€2,492+ (Loose Maarten CommV dominates — TruSecure + 5 others have zero confirmed open invoices this run).**
```
