# Cron Job: Weekly Timesheet Rollup

**Job ID:** e4010278f409
**Run Time:** 2026-05-22 15:10:04
**Schedule:** 0 15 * * 5
...
Create the weekly timesheet rollup for the Clawbot Obsidian vault at `/Users/absorbo/Library/Mobile Documents/com~apple~CloudDocs/AgentVault/Clawbot`.

Tasks:
1. Find Monday-Friday daily notes for the current week.
2. Extract `## 📋 Timesheet` table rows from each day.
3. Aggregate per customer: sum hours and collect work descriptions.
4. Create `01 - Daily Notes/YYYY-Www.md` using the format from `90 - Templates/Timesheet-Weekly.md`.
5. Flag gaps: zero-hour customers, zero-hour days, unresolved `[ESTIMATE]` entries.
6. Report: number of customers, total hours, logged/missing customers.

Constraints: never delete files; never modify templates or archives; never fabricate hours; preserve `[ESTIMATE]`/`[CONFIRM]` uncertainty markers.
...
**Week 21 timesheet rollup complete.**

| Metric | Value |
|--------|-------|
| **Created** | `01 - Daily Notes/2026-W21.md` |
| **Changed** | 0 files |
| **Skipped** | 0 |

**Numbers:**
- **3 customers** with hours: Delvaux (0.75), Groep Brugge (4.5), Verlinfo (0.5)
- **4 customers** at zero: CAPAC-HVW (missed capture), Inetum, Futurwork, Admin
- **Total: 5.75h** across 6 entries

**⚠️ Needs attention:**
- **All 6 entries are `[ESTIMATE]`** — 0 confirmed. User has not reviewed any daily timesheet this week.
- **CAPAC-HVW missed capture**: task completed Monday but never logged. Flagged Tuesday, still unresolved.
- **Friday has no timesheet section** — today's standup + prep call (11:00–12:00 with Koen) not yet recorded. Re-run after Friday note is populated.
- **4 open `[CONFIRM]` markers** carried forward from daily notes.
