...
| **Calendar** | ❌ Not supported | Use `google-workspace` skill instead |
| **Contacts** | ❌ Not supported | — |
| **OneDrive** | ❌ Not supported | — |
| **User Profile** | ❌ Not supported | — |

**Backend:** `scripts/m365_graph_mail.py` — Python CLI using MSAL with PKCE for mail operations.

> ⚠️ **Note:** This skill currently only supports email/mail operations. Calendar events can be retrieved using the `google-workspace` skill (`$GAPI calendar list`).
...
2. **No calendar support:** This script only handles mail. For calendar events, use `google-workspace` skill.
...
The daily note has been successfully created. Here is the status report:

---

**Morning Briefing Status — 2026-05-27 (Wednesday)**

**Created:** Daily note `01 - Daily Notes/2026-05-27.md` ✅

**Email Summary:**
- Gmail: 7 unread triaged
- M365: 14 unread triaged
- Total triaged: 21

**Yesterday's EOD Follow-Up:** Skipped — no EOD feedback content in yesterday's note (only placeholder).

**🔺 Action Required (3 tasks):**
1. **Pay overdue Enwyse invoice F-2026020482 (€581,22)** — Final notice with collection threat (€87,18 incassokosten). Due 22-04-2026 (overdue 2 months). Transfer to BE64 7330 7189 8652.
2. **Pay Enwyse invoice F-2026027153 (€549,14)** — First reminder, due yester. Same account.
3. **Review Google Trust Services TLS changes** — ECDA certificate rotation before Jun 15. Only relevant if using custom trust stores/pinning.

**⏫ Important (3 items):**
- Productive time reminder
- TruSecure platform notifications to check
- Karsten/bol BTW-factuur for shadow sail purchase

**🔼 FYI (6 items):**
- GRC Report (Adani sanctions), ComplyCube promo, Mitratech AI explainability (10% confident), GRC2020 newsletter, AI Snacks newsletter, Vinted bid notices

**🔽 Ignored:** HighLevel promo, OneCal reactivation (already triaged yesterday), Sunweb review, Enwyse marketing

**Drafts created:** None needed today. OneCal draft exists in yesterday's note (was addressed there).

**Blockers:** Enwyse invoices require payment decision — overdue by 1–2 months, collection action pending.
