...
## Content Pipeline Health Scan — Tuesday 2026-06-02

### 🚨 ALERT: Schedulable content below minimum (3rd consecutive daily scan)

| Metric | Current | Minimum | Status |
|--------|---------|---------|--------|
| **Schedulable (next 14 days)** | **0** | ≥ 4 | 🔴 **-4 gap** |
| Drafted pipeline (beyond 14 days) | 30 | ≥ 8 | 🟢 Healthy |

### Root cause
Every item in **Approved.md** (5 items) and **Scheduled.md** (2 items) has a target date in the past — all before May 7, 2026. The pipeline is publishing nothing between June 2 and June 16. Three Approved items (W1-LI-B, W1-FB-B, W2-FB-B) are stale approvals that were never moved to Posted or rescheduled.

### What's healthy
- **11 full drafts** ready for approval in [[60 - Content-Pipeline/Drafts|Drafts.md]] — the largest queue the pipeline has had. Two case-style W2 drafts (incident response + green dashboard) were added this morning at 08:06–08:10 UTC, superseding the previous W2 A-slots per Monday's theme choice.
- **19 ideas** in [[60 - Content-Pipeline/Ideas|Ideas.md]] backlog.
- LinkedIn drafts remain teach-first.

### Additional flags
- ⚠️ **39 days since last post** (Apr 24 → Jun 2). Far over the 2×/week baseline.
- ⚠️ Structural problem is **approval**, not production. 11 drafts ready, 0 approved in over a month.
- ⚠️ Saturday → Sunday → Tuesday (Mon was skipped per cron policy). 3rd consecutive daily scan finding the same gap.

### Recommended action
1. **Approve the 2 case-style W2 drafts** (W2-LI-A incident response, W2-FB-A green dashboard) — they are theme-aligned with Monday 2026-06-01's chosen bucket and unblock the A-slots.
2. **Approve 2 more** from W3 (LI-A, FB-A, LI-B, FB-B) to reach the minimum 4 schedulable.
3. **Clean up the stale queue**: move W1-LI-B / W1-FB-B / W2-FB-B from Approved/Scheduled to Posted (if published) or reschedule with fresh dates.
4. **Add a recurring Thursday approval session** per the MOC's Weekly Rhythm — drafts accumulate but the gate is broken.

Alert appended to today's daily note at `01 - Daily Notes/2026-06-02.md` under `## ⚠️ Content Pipeline Alert`.
