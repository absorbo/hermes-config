...
**Content Pipeline Health — ALERT (Saturday 2026-06-06)**

State verified against `60 - Content-Pipeline/{Content Pipeline MOC, Approved, Scheduled, Drafts, Ideas, Posted}.md`. The pipeline is below the schedulable minimum — alert written.

| Metric | Current | Minimum | Gap |
|---|---|---|---|
| Schedulable (next 14 days) | **0** | ≥ 4 | **−4** |
| Drafted pipeline | 30 (11 drafts + 19 ideas) | ≥ 8 | ✅ +22 |

**Why alerting:** Schedulable = 0. All three actionable Approved items (W1-LI-B, W1-FB-B, W2-FB-B) carry April–May target dates that have already passed; they were never moved to Posted and never rescheduled. Scheduled.md staging is also stale (Apr 27 / Apr 29). Zero items are ready to publish between 2026-06-06 and 2026-06-20, so the rolling-horizon rule (≥ 2 weeks approved/schedulable) is fully breached.

**Drafted pipeline is healthy.** 11 drafts in Drafts.md (5 LinkedIn, 6 Facebook) and 19 ideas in Ideas.md (10 ready-to-draft, 6 commercial, 3 explore). The bottleneck is approval, not production.

**Additional quality checks (Step 5):**
- LinkedIn teach-first: ✅ All 5 LinkedIn drafts in queue lead with teaching; commercial CTAs live on Facebook.
- Ideas backlog ≥5: ✅ 19 items.
- >7 days since last post: ❌ **43 days** since W1-LI-A / W1-FB-A on 2026-04-24.

**Action this job did NOT take:** No approvals, no scheduling, no draft generation. The gate stays on Maarten.

**Action filed to today's daily note:** Appended a `### 2026-06-06 cron run (verified)` sub-section under the existing `## ⚠️ Content Pipeline Alert` heading in `01 - Daily Notes/2026-06-06.md`. The single highest-leverage move remains 4× "approve" replies to the W2 + W3 drafts listed in `## 📝 Content Approval Queue` — that fills the 2-week schedulable horizon in one sitting.
